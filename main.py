                ...
                if st.button("**Clear Duplicates**"):
                    ...
                    while True:
                        ...
                        for col in selectedcols:
                            ...
                            if len(data["Team No."]) > 1 and data[col][row] == data[col][row-1]:
                                isduplicate = True
        ...
        if editchoice == "Remove a Question":
            ...
            if qset == "Pit":
                ...
                if len(st.session_state.pitq) > 2:
                    ...
                    if len(st.session_state.pitq) == 0:
                        st.subheader(f"No {qset} Items Added Yet.")
...
if st.session_state.admin:
    ...
    exgit = sidebar.expander("**:blue[GitHub] Actions**")
    ...
    if exgit.button("**Pull From :blue[GitHub]**", use_container_width=True):
        ...
        try:
            ...
            gitpull()
            savedata(st.session_state.pitdata, st.session_state.matchdata)
            st.success("**Successfully retrieved data from :blue[GitHub].**")
    ...
    # MinIO code backup - no longer in use
    miniobkp = """
    exminio = sidebar.expander("**:red[MINIO] Actions**")

    exminio.header("**:red[MinIO]** Actions:")

    if exminio.button("**Load Data**", use_container_width=True):
    ...
    if exminio.button("**Save Data**", use_container_width=True):
    ...
    if ex2.button("**:red[Clear] System Log**"):
        os.system("cls")
        os.system("clear")
        print("""\033[1m
-------------------
